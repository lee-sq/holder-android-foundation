package com.holderzone.network.interceptor
 ...
 * 日志打印拦截器
 *
 * 源码来自 https://github.com/square/okhttp/blob/master/okhttp-logging-interceptor/src/main/java/okhttp3/logging/HttpLoggingInterceptor.java
 * 源码中每一行均使用logger.log()打印信息
 * 但本项目中使用的是带有格式的日志工具，会造成每打印一行消息都带上了格式
 * 因此采用字符串拼接的方式，只在request和response这两处各自拼接完成后，使用logger.log()打印
 *
 * Json字符串格式化策略，Formatter参照如下项目：
 * https://github.com/parkingwang/okhttp3-loginterceptor
 *
 * @author terry
 * @date 2018/08/21 下午6:46
 */
